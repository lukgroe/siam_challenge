function simObj = mixed_strategy(simObj, mu, c) % mu only given to compare approximation to exact value.
    ...
    for i=1:simObj.T
        ...
        for j = 1:(simObj.d)
            ...
            for k=1:min(w_zero_counter-1, w_nonzero_counter-1)
                c_t(j) = c_t(j) + (s_w_delta_zero(j,k)-s_w_delta_nonzero(j,k))/w_w_delta_nonzero(j,k);
            end
        end
